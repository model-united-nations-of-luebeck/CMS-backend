...
def registration_list(request):
    ...
    for school in School.objects.all():
        ...
        for mun_director in mun_directors:
            story.append(Paragraph(f"{mun_director.first_name} {mun_director.last_name} <font size=8>({mun_director.pronouns})</font>  <i>{mun_director.email}, {mun_director.mobile}</i>", cell_style))
        ...
        # Table
        table = Table(data, colWidths=[60*mm, 60*mm, 30*mm, 35*mm], repeatRows=1)
