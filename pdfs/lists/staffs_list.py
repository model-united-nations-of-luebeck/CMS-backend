...
def staffs_list(request):
    ...
    # Title
    story.append(Paragraph("Staff List", styles["Title"]))
    ...
    staffs = Staff.objects.all().order_by('position_name', 'first_name')
    for staff in staffs:
        data.append([
    ...
    buffer.seek(0)
    return FileResponse(FileWrapper(buffer), filename='staffs_list.pdf', content_type="application/pdf", as_attachment=False)
