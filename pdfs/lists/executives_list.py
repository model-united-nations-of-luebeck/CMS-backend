...
from api.models import Delegate, Executive, Forum, Issue, MemberOrganization, School
...
def executives_list(request):
    ...
    # Title
    story.append(Paragraph("Executive Staff Team", styles["Title"]))
    ...
    executives = Executive.objects.all().annotate(reverse_position_name=Reverse('position_name')).order_by('reverse_position_name')
    for exec in executives:
        data.append([
            Paragraph(f"{exec.first_name} {exec.last_name}", cell_style),
            Paragraph(exec.position_name, cell_style),
            Paragraph(exec.email, cell_style),
            Paragraph(str(exec.mobile), cell_style),
        ])
    ...
    buffer.seek(0)
    return FileResponse(FileWrapper(buffer), filename='executives_list.pdf', content_type="application/pdf", as_attachment=False)
