...
from api.models import Delegate, Executive, Forum, Issue, MemberOrganization, Participant, School, Staff, StudentOfficer
...
def allergy_list(request):
    ...
    # Title
    story.append(Paragraph("Allergy List", styles["Title"]))
    story.append(Spacer(1, 12))

    # Table data
    data = [
        ["Name", "Provided additional information (including allergies)"]
    ]

    participants = Participant.objects.all().order_by('first_name', 'last_name')
    for participant in participants:
        if participant.extras is None:
            continue

        data.append([
            Paragraph(f"{participant.first_name} {participant.last_name}", cell_style),
            Paragraph(participant.extras, cell_style),
        ])

    # Table
    table = Table(data, colWidths=[60*mm, 200*mm], repeatRows=1)
    ...
    buffer.seek(0)
    return FileResponse(FileWrapper(buffer), filename='allergy_list.pdf', content_type="application/pdf", as_attachment=False)
