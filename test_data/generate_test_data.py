...
def generate_advisors(n:int=20):
    ...
    for _ in range(n):
        advisor = Advisor.objects.get_or_create(first_name=fake.first_name(), last_name=fake.first_name(), gender=select_randomly_from_choices(Person.GENDER_CHOICES), email=fake.email(), mobile=generate_valid_phone_number(), birthday=fake.date(), extras=fake.text(), car=random.choice([True, False]), availability=fake.text(), experience=fake.text())
...
def generate_staffs(n:int=40):
    ...
    for _ in range(n):
        ...
        staff = Staff.objects.get_or_create(first_name=fake.first_name(), last_name=fake.first_name(), gender=select_randomly_from_choices(Person.GENDER_CHOICES), email=fake.email(), mobile=generate_valid_phone_number(), birthday=fake.date(), extras=fake.text(),position_name=random.choice(position_names), school_name=fake.name()+ "-School")
...
def generate_mun_directors(n:int=50):
    ...
    for _ in range(n):
        ...
        mun_director = MUNDirector.objects.get_or_create(first_name=fake.first_name(), last_name=fake.first_name(), gender=select_randomly_from_choices(Person.GENDER_CHOICES), email=fake.email(), mobile=generate_valid_phone_number(), birthday=fake.date(), extras=fake.text(), english_teacher=random.choice([True, False]), school=random.choice(School.objects.all()))
...
def generate_student_officers(n:int=30):
    ...
    for _ in range(n):
        ...
        student_officer = StudentOfficer.objects.get_or_create(first_name=fake.first_name(), last_name=fake.first_name(), gender=select_randomly_from_choices(Person.GENDER_CHOICES), email=fake.email(), mobile=generate_valid_phone_number(), birthday=fake.date(), extras=fake.text(), position_name=random.choice(position_names), position_level=random.choice([True, False]), school_name=fake.name()+"-School", forum=random.choice(Forum.objects.all()))
...
def generate_delegates(n:int=250):
    ...
    for _ in range(n):
        ...
        delegate = Delegate.objects.get_or_create(first_name=fake.first_name(), last_name=fake.first_name(), gender=select_randomly_from_choices(Person.GENDER_CHOICES), email=fake.email(), mobile=generate_valid_phone_number(), birthday=fake.date(), extras=fake.text(), ambassador=random.choice([True, False]), represents=random.choice(MemberOrganization.objects.all()), school=random.choice(School.objects.all()), forum=random.choice(Forum.objects.all()))